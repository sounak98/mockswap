...
contract Pair is ERC20, IPair {
    ...
    constructor(address _tokenA, address _tokenB) ERC20("Pair", "PAIR") {
        // check constructor_tokenA_is_not_a_contract_address
        require(iscontract(_tokenA), "TOKENA_IS_NOT_CONTRACT");
        // check constructor_tokenB_is_not_a_contract_address
        require(iscontract(_tokenB), "TOKENB_IS_NOT_CONTRACT");
        // check constructor_tokenA_tokenB_identical_address
        require(_tokenA != _tokenB, "TOKENA_TOKENB_IDENTICAL_ADDRESS");
    ...
    function swap(
        ...
        require(tokenOut == tokenA || tokenOut == tokenB, "INVALID_TOKEN_OUT");
        // when_tokenIn_is_the_same_as_tokenOut
        require(tokenIn != tokenOut, "IDENTICAL_ADDRESS");
        ...
        // check x*y=k
        require(balanceA * balanceB == reserveA * reserveB, "K");
    ...
    function addLiquidity(
        ...
        virtual
        override
        returns (uint256 _amountA, uint256 _amountB, uint256 liquidity)
    {
        // revert when_tokenA_amount_is_zero
        require(amountAIn > 0, "INSUFFICIENT_INPUT_AMOUNT");
        // revert when_tokenB_amount_is_zero
        require(amountBIn > 0, "INSUFFICIENT_INPUT_AMOUNT");
        // calculate liquidity
    ...
    function removeLiquidity(
        uint256 liquidity
    ) external virtual override returns (uint256 _amountA, uint256 _amountB) {
    ...
    function mint(address _to) internal lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            liquidity = Math.min(
                (amountA_ * _totalSupply) / reserveA,
                (amountB_ * _totalSupply) / reserveB
            );
        }
    ...
    function burn(
        address _to
    ) internal lock returns (uint _amountA, uint _amountB) {
        uint balanceA = IERC20(tokenA).balanceOf(address(this));
        uint balanceB = IERC20(tokenB).balanceOf(address(this));
        ...
        require(_amountA > 0 && _amountB > 0, "INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function quote(
        ...
        uint _reserveA,
        uint _reserveB
    ) internal pure returns (uint _amountB) {
